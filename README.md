...
When available, the connector caches raw responses for each unique [cache key](#cache-key).
...
The default cache key is comprised of the source and options parameters passed to `Connector::fetch`. Options are sorted before the cache key is created so the order of options are insignificant.

#### Example

```php
class SingleCacheKeyGenerator implements CacheKeyGeneratorInterface
{
    public function generateCacheKey($source, EncapsulatedOptions $options) {
        return 'KEY';
    }
}
