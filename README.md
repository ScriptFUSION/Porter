...
When available, the connector caches raw responses for each unique [cache key](#cache-key). Cache keys are generated by an implementation-defined strategy or the default `SourceAndOptionsHashCacheKeyGenerator` strategy.
...
The cache key can optionally be generated by an implementation of `CacheKeyGeneratorInterface` passed into `CachingConnector::fetch`. This implementation should provide one method `generateCacheKey` which returns a [PSR-6][PSR-6]-compatible cache key.

The default implementation `SourceAndOptionsHashCacheKeyGenerator` generates keys comprised of the source and options parameters passed to `Connector::fetch`. Options are sorted before the cache key is created so the order of options are insignificant.

#### Example

```php
class SourceCacheKeyGenerator implements CacheKeyGeneratorInterface
{
